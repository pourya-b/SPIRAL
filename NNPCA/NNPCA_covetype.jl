...
using ProximalAlgorithms: IterationTools, LBFGS
...
data = CSV.read("../../../../datasets/covtype.scale.csv", DataFrame)

data = Matrix(data)
println(typeof(data))
# data = data[2:end,2:end] # removing labels (MNIST)
data = data[2:end,1:end-1] # removing labels (MNIST)
...
g = ProximalOperators.IndNonnegativeBallL2()
...
##############################################################################
convex_flag = false
maxit = 100 |> Int # maximum number of epochs (not exact for lbfgs)
