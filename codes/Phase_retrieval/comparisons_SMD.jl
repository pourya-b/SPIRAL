...
function getresidue!(iter, tol, factor, Maxit, freq, γ::Array{Float64}, res_xz, H)
...
function Comparisons_SMD!(
    ...
    iter = BregmanBC.iterator(solver, x0, F = func.F, g = func.g, H = H[1], L = mean(L), N = N)
    dist_hist, it_hist, z_sol, dist2_hist, cost_hist  = getresidue!(iter, tol, factor, Maxit, freq, γ, res_xz, H)
